...
void find_expression(int size,int *curr_list, int i, int value, char curr_string_vector[][100], char *ans, int flag, int combinations[][2], int* tcalls)
{
    ...
    if(size == 0)
    {
        ...
        {
            strcpy(ans, curr_string_vector[0]); // copia a resposta se for encontrada uma express�o que chega no valor
        }
         return;
    }
    if(flag == 0) // gera uma nova combina��o para cada etapa, cada novo tamanho
    {
    ...
    find_expression(size,curr_list,i+1,value,curr_string_vector,ans,1,combinations, tcalls); // considera todos as combina��es poss�veis para cada etapa
